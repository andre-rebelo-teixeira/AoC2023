...
Rank decode_card_type(std::string play)
{
    ...
    unsigned short int number_of_j = card_to_count.find('J') != card_to_count.end() ? card_to_count['J'] : 0;
    card_to_count.erase('J');

    // get card more frequent
    auto max_element = std::max_element(card_to_count.begin(), card_to_count.end(), [](auto &left, auto &right) -> bool
    ...
    max_element->second += number_of_j;
